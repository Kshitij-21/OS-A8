...
int main(int argc, char *argv[])
{
    ...
    SM1 = (PageTable *)shmat(shm1, NULL, 0);
    ...
    {
        timestamp++;
        msgrcv(msg2_shm, (void *)&message2, sizeof(message_type_2), 1, 0);
        ...
        fprintf(fp,"MMU: Global Ordering(pid-%d) - (Timestamp %d, Process %d, Page %d)\n", SM1[i].pid, timestamp, i + 1, message2.pageorframe);
        fflush(fp);
        ...
        else if (page >= SM1[i].pages_req)
        {
            ...
            fprintf(fp,"MMU: Invalid Page Reference(pid-%d) - (Process %d, Page %d)\n", SM1[i].pid, i + 1, page);
            fflush(fp);
        ...
        else
        {
            ...
            fprintf(fp,"MMU: Page fault sequence(pid-%d)- (Process %d, Page %d)\n", SM1[i].pid, + +1, page);
            fflush(fp);
            int j = 0;
            while (1)
            {
